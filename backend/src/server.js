...
app.post('/api/gifts/send',auth,async(req,res)=>{
  ...
  const c=await pool.connect(); try {
    await c.query('begin');
    const existing=(await c.query(`select * from gift_transactions where idempotency_key=$1`,[idempotencyKey])).rows[0];
    if(existing){ await c.query('commit'); return res.status(200).json(existing); }
    const g=(await c.query(`select * from gifts where id=$1 and active=true`,[giftId])).rows[0]; if(!g) throw new Error('GIFT_NOT_FOUND'); const qty=Number(quantity);
    ...
    const total=BigInt(g.price)*BigInt(qty); const bal=(await c.query(`select coalesce(sum(amount),0)::bigint balance from wallet_ledger where user_id=$1 and currency='coins'`,[req.user.sub])).rows[0].balance; if(BigInt(bal)<total) throw new Error('INSUFFICIENT_COINS'); await c.query(`insert into wallet_ledger(user_id,currency,amount,type,reference_id,metadata) values($1,'coins',$2,'gift_spend',$3,$4),($1,'diamonds',$5,'gift_receive',$7,$6)`,[req.user.sub,(-total).toString(),idempotencyKey+':spend',JSON.stringify({roomId,receiverId,giftId,quantity:qty}),total.toString(),JSON.stringify({roomId,senderId:req.user.sub,giftId,quantity:qty}),idempotencyKey+':receive']); const t=await c.query(`insert into gift_transactions(sender_id,receiver_id,room_id,gift_id,quantity,unit_price,total_price,idempotency_key) values($1,$2,$3,$4,$5,$6,$7,$8) returning *`,[req.user.sub,receiverId||null,roomId,giftId,qty,g.price,total.toString(),idempotencyKey]); await c.query('commit'); io.to(`room:${roomId}`).emit('room:gift',t.rows[0]); res.status(201).json(t.rows[0]); } catch(e){ await c.query('rollback'); res.status(400).json({error:e.message}); } finally { c.release(); }
});
